...
3. **Queries use `pgx.NamedArgs` with snake_case keys** and the shared builders from `common.go`:
   ...
   - Reads: explicit `SELECT` with aliased columns (`FROM users AS u WHERE u.email=$1`) or `SELECT *` for full-struct scans; fetch via `pgxscan.Get(ctx, conn, &dst, query, args...)`.
